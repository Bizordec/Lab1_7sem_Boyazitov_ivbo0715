...
class FreqAnalysis {
    ...
    private static int countEngLetters = 0;
    private static int countLetter[] = new int[alphSize];
    private static int countEngBigrams = 0;
    ...
    private static List<Map.Entry<Character, Integer>> freqAlgorithm(List<char[]> textList) {
            ...
            for (char letter : string) {
                for (int b = 0, e = alphSize - 1; b < alphSize || e > 0; b++, e--) {
                    if (letter == alphabet[b] || letter == alphabet[e]) {
                        countEngLetters++;
                        letterMap.put(letter, ++countLetter[letter - firstLetter]);
                        break;
                    }
                }
            }
        ...
        System.out.println("Number of English letters in the text: " + countEngLetters);
        for (int i = 0; i < letterEntryList.size(); i++) {
            double freq = 100.0 * letterEntryList.get(i).getValue() / countEngLetters;
    ...
    private static List<Map.Entry<String, Integer>> freqBigramAlgorithm(List<char[]> textList) {
        ...
        System.out.println("Number of bigrams in the text: " + countEngBigrams);
        for (int i = 0; i < bigramEntryList.size(); i++) {
            double freq = 100.0 * bigramEntryList.get(i).getValue() / countEngBigrams;
            String formattedFreq = new DecimalFormat("#0.00").format(freq);
